...
use inclass3;
create table Bank_Inventory_pricing(Product char(15),Quantity int, Price real,purchase_cost decimal(6,2),
estimated_sale_price float,month int); 
...
select Purchase_cost from Bank_inventory_pricing where month=2;
...
select Product,count(*),month from bank_inventory_pricing where month =1 group by product having count(*)>1 ;
...
select product,count(*),purchase_cost,estimated_sale_price from bank_inventory_pricing where purchase_cost > ifnull(estimated_sale_price,0) group by product having count(*)>1;
...
select month,sum(purchase_cost) from bank_inventory_pricing where month<3 group by month;
...
select product,avg(estimated_sale_price) from bank_inventory_pricing where purchase_cost like null group by product;
...
select product,max(estimated_sale_price) from bank_inventory_pricing where quantity>4 and purchase_cost is not null group by product;
...
select product, avg(estimated_sale_price)+((avg(estimated_sale_price)/100)*15) 
...
select product,avg(ifnull(purchase_cost,estimated_sale_price)) from bank_inventory_pricing where product='BusiCard' group by product;
...
select product, avg(price) from bank_inventory_pricing where month=1 and month =2 and month =3 group by product;
