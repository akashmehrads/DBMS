...
/* Solution */
select product, month, quantity from bank_inventory_pricing group by product,month;
# Question 4:
# 4)  Find the maximum profit(Estimated_sale_price-Purchase_cost) for each product (Hint: Do not display loss).
/* Solution */
select product,max(estimated_sale_price-purchase_cost) profit from bank_inventory_pricing group by product;
# Question 5:
# 5)  Find the minimum revenue for each banker if estimated_profit is greater than 20000000.
/* Solution */
select banker, min(revenue) from bank_branch_pl group by banker;
...
select branch, revenue-cost Real_Profit, estimated_profit from bank_branch_pl where revenue-cost>estimated_profit;
# Question 8:
...
select branch, sum(estimated_profit),sum(revenue)-sum(cost) from bank_branch_pl group by branch;
# Question 9:
...
select branch, product,revenue-cost from bank_branch_pl order by revenue-cost limit 0,1;
# Question 11:
...
select branch,product from bank_branch_pl where revenue-cost>estimated_profit; 
# Question 12:
...
select product, revenue-cost,estimated_profit from bank_branch_pl where estimated_profit-revenue-cost<estimated_profit/10; 
# Question 13:
...
select branch,product from bank_branch_pl where estimated_profit<revenue-(cost-cost/100*30); 